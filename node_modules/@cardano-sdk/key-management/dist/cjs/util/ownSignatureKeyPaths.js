...
const checkStakeCredential = (address, keyHash) => address.stakeKeyDerivationPath && core_1.Cardano.RewardAccount.toHash(address.rewardAccount) === keyHash
...
const checkPaymentCredential = (address, keyHash) => {
    const paymentCredential = core_1.Cardano.Address.fromBech32(address.address)?.asBase()?.getPaymentCredential();
...
const combineSignatureChecks = (a, b) => ({
...
const processSignatureScript = (script, groupedAddresses) => {
    let signatureCheck = { derivationPaths: [], requiresForeignSignatures: false };
    for (const address of groupedAddresses) {
...
const getNativeScriptKeyPaths = (groupedAddresses, nativeScripts) => {
    const signatureCheck = { derivationPaths: [], requiresForeignSignatures: false };
    if (!nativeScripts?.length)
        return signatureCheck;
    const processScript = (script) => {
        if (!core_1.Cardano.isNativeScript(script)) {
            return { derivationPaths: [], requiresForeignSignatures: false };
        }
        switch (script.kind) {
            case core_1.Cardano.NativeScriptKind.RequireSignature: {
                return processSignatureScript(script, groupedAddresses);
            }
            case core_1.Cardano.NativeScriptKind.RequireAllOf:
            case core_1.Cardano.NativeScriptKind.RequireAnyOf:
            case core_1.Cardano.NativeScriptKind.RequireNOf: {
                const scriptWithScripts = script;
                return scriptWithScripts.scripts.reduce((acc, subScript) => combineSignatureChecks(acc, processScript(subScript)), { derivationPaths: [], requiresForeignSignatures: false });
            }
            case core_1.Cardano.NativeScriptKind.RequireTimeBefore:
            case core_1.Cardano.NativeScriptKind.RequireTimeAfter:
                return { derivationPaths: [], requiresForeignSignatures: false };
            case core_1.Cardano.NativeScriptKind.RequireGuard: {
                const guardScript = script;
                return guardScript.credential.type === core_1.Cardano.CredentialType.KeyHash
                    ? processSignatureScript({
                        __type: core_1.Cardano.ScriptType.Native,
                        keyHash: guardScript.credential.hash,
    ...
    return nativeScripts.reduce((acc, script) => combineSignatureChecks(acc, processScript(script)), signatureCheck);
};
...
const ownSignatureKeyPaths = (txBody, knownAddresses, txInKeyPathMap, dRepKeyHash, scripts) => {
