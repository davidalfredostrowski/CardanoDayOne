(function (stream, module, chai, fs) {
	...
	function read() {
		...
		if (!(position$1 < srcEnd)) throw endOfCBORError()
		...
		switch (majorType) {
			...
			case 4: // array
				...
				if (token > srcEnd - position$1) throw endOfCBORError()
			...
			case 5: // map
				...
				if (token > (srcEnd - position$1) / 2) throw endOfCBORError() // each entry needs at least two bytes
				if (currentDecoder.mapsAsObjects) {
					let object = {};
	...
	if (!nativeAccelerationDisabled) {
		...
		try {
			if (typeof require == 'function')
				...
				extractor = module.createRequire((_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('test.js', document.baseURI).href))('cbor-extract');
			if (extractor)
	...
	const sampleData = JSON.parse(fs.readFileSync(new URL('./example4.json', (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('test.js', document.baseURI).href))));
	...
	suite('CBOR basic tests', function(){
		...
		test('string overflow', function() {
			assert.throws(() => decode$1(Buffer.from('7a10000000', 'hex')));
		});

		test('overlong encoding', function() {
			const payload = Buffer.from([0x62, 0xc0, 0xaf]);
			const result = decode$1(payload);
