...
function decodeBase32(base32Str, alphabet) {
    if (!Array.from(base32Str).every(function (ch) { return alphabet.includes(ch); }))
    ...
    for (var i = 0; i < len - 1; i++) {
        var num_1 = alphabet.indexOf(base32Str[i].toLowerCase());
        bits += num_1.toString(2).padStart(5, '0');
    }
    // last, make sure we align to byte
    var nCut = len * 5 - 8 * Math.floor(len * 5 / 8);
    var num = alphabet.indexOf(base32Str[len - 1].toLowerCase());
