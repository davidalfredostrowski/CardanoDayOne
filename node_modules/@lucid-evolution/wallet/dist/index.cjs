...
__export(index_exports, {
  discoverOwnUsedTxKeyHashes: () => discoverOwnUsedTxKeyHashes,
...
function walletFromSeed(seed, options = {}) {
  ...
  const entropy = (0, import_bip39.mnemonicToEntropy)(seed);
  const rootKey = CML.Bip32PrivateKey.from_bip39_entropy(
    (0, import_core_utils.fromHex)(entropy),
...
function discoverOwnUsedTxKeyHashes(tx, ownKeyHashes, ownUtxos) {
  ...
  for (let i = 0; i < inputs.len(); i++) {
    ...
    if (utxo) {
      const { paymentCredential } = (0, import_utils.getAddressDetails)(utxo.address);
...
var import_sign_data = require("@lucid-evolution/sign_data");
var signTxsSequentially = async (txs, signTx) => {
  const witnessSets = [];
  for (const tx of txs) {
...
var submitTxsSequentially = async (txs, submitTx) => {
  const results = [];
...
var decodeWitnessSets = (witnessSets, expectedLength) => {
  if (witnessSets.length !== expectedLength) {
    throw new Error("Wallet returned an unexpected number of witness sets.");
  }
  return witnessSets.map(
    (witnessSet) => CML.TransactionWitnessSet.from_cbor_hex(witnessSet)
  );
};
var validateTxHashes = (txHashes, expectedLength) => {
...
var makeWalletFromSeed = (provider, network, seed, options) => {
  ...
  const signTx = async (tx) => {
    const utxos = config.overriddenUTxOs !== void 0 ? config.overriddenUTxOs : await provider.getUtxos(address);
  ...
  return {
    ...
    clearUTxOOverride: () => config.overriddenUTxOs = void 0,
    address: async () => address,
    rewardAddress: async () => rewardAddress || null,
    getUtxos: async () => config.overriddenUTxOs !== void 0 ? config.overriddenUTxOs : provider.getUtxos(address),
    getUtxosCore: async () => {
      const utxos = config.overriddenUTxOs !== void 0 ? config.overriddenUTxOs : await provider.getUtxos(address);
      const coreUtxos = [];
      for (const utxo of utxos) {
        coreUtxos.push((0, import_utils2.utxoToCore)(utxo));
      }
      return coreUtxos;
    },
    getDelegation: async () => {
    ...
    signTx,
    signTxs: async (txs) => signTxsSequentially(txs, signTx),
    signMessage: async (address2, payload) => {
      const {
        paymentCredential,
        stakeCredential,
        address: { hex: hexAddress }
      } = (0, import_utils2.getAddressDetails)(address2);
      ...
      return (0, import_sign_data.signData)(hexAddress, payload, privateKey);
    },
    submitTx: async (tx) => provider.submitTx(tx),
    submitTxs: async (txs) => submitTxsSequentially(txs, (tx) => provider.submitTx(tx))
  };
};
...
var makeWalletFromAPI = (provider, api) => {
  ...
  return {
    ...
    getUtxos: async () => {
      const utxos = config.overriddenUTxOs !== void 0 ? config.overriddenUTxOs : (await api.getUtxos() || []).map(
        (utxo) => (0, import_utils2.coreToUtxo)(
          CML.TransactionUnspentOutput.from_cbor_bytes((0, import_core_utils2.fromHex)(utxo))
        )
      );
      return utxos;
    },
    ...
    signTx: async (tx) => {
      const witnessSet = await api.signTx((0, import_core_utils2.toHex)(tx.to_cbor_bytes()), true);
      return CML.TransactionWitnessSet.from_cbor_hex(witnessSet);
    },
    signTxs: async (txs) => {
      const cbors = txs.map((tx) => (0, import_core_utils2.toHex)(tx.to_cbor_bytes()));
      const requests = cbors.map((cbor) => ({
        cbor,
        partialSign: true
      }));
      const signTxs = api.cip103?.signTxs?.bind(api.cip103) ?? api.experimental?.signTxs?.bind(api.experimental) ?? api.signTxs?.bind(api);
    ...
    signMessage: async (address, payload) => {
      const hexAddress = (0, import_core_utils2.toHex)(CML.Address.from_bech32(address).to_raw_bytes());
    ...
    submitTxs: async (txs) => {
      ...
      const txHashes = submitTxs ? await submitTxs(txs) : await submitTxsSequentially(txs, (tx) => api.submitTx(tx));
      return validateTxHashes(txHashes, txs.length);
    }
  };
};
var makeWalletFromAddress = (provider, network, address, utxos) => {
  const { stakeCredential } = (0, import_utils2.getAddressDetails)(address);
  const rewardAddress = stakeCredential ? (0, import_utils2.credentialToRewardAddress)(network, stakeCredential) : null;
  const config = {
  ...
  return {
    ...
    getUtxos: async () => config.overriddenUTxOs !== void 0 ? config.overriddenUTxOs : provider.getUtxos(address),
    getUtxosCore: async () => {
      const utxos2 = config.overriddenUTxOs !== void 0 ? config.overriddenUTxOs : await provider.getUtxos(address);
      const coreUtxos = [];
