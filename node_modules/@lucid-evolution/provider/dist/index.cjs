...
__export(index_exports, {
  ...
  Maestro: () => Maestro,
  OgmiosJsonRpcError: () => OgmiosJsonRpcError,
...
var retryAfter = (cause) => {
  ...
  return retryAfterHeaderToMs(typeof value === "string" ? value : void 0);
};
var errorTag = (cause) => {
  const record = asRecord(cause);
  return [record?._tag, record?.name, record?.reason].filter((value) => typeof value === "string").join(" ").toLowerCase();
};
var inferKind = (cause, status) => {
  const tag = errorTag(cause);
  if (tag.includes("timeout")) return "timeout";
  if (tag.includes("parse") || tag.includes("decode")) return "decode";
  if (status !== void 0) return "http";
  if (tag.includes("request") || tag.includes("transport")) return "transport";
  return "unknown";
};
var inferRetryable = (kind, status) => kind === "timeout" || kind === "transport" || status === 408 || status === 425 || status === 429 || status !== void 0 && status >= 500;
var KupmiosError = class extends Error {
  _tag = "KupmiosError";
  provider = "Kupmios";
  protocol;
  operation;
  ...
  constructor(options) {
    ...
    const statusSuffix = status === void 0 ? "" : ` (HTTP ${status})`;
    super(
      options.message ?? `${options.protocol ?? "provider"} ${operation} failed${statusSuffix}`,
      {
        cause: options.cause
      }
    );
...
var OgmiosJsonRpcError = class extends KupmiosError {
  _tag = "OgmiosJsonRpcError";
  code;
  data;
  method;
  id;
  constructor(options) {
    const dataSuffix = options.data === void 0 ? "" : `: ${JSON.stringify(options.data)}`;
    super({
      protocol: "ogmios",
...
var toKupmiosError = (cause, protocol, operation) => cause instanceof KupmiosError ? cause : new KupmiosError({ cause, protocol, operation });
...
var package_default = {
  ...
  dependencies: {
    ...
    "@anastasia-labs/cardano-multiplatform-lib-nodejs": "6.2.0-1",
...
var fromAssets = (assets) => (0, import_effect.pipe)(import_effect.Record.remove(assets, "lovelace"), (nativeAssets) => {
  const result = {};
...
// src/blockfrost.ts
var toBlockfrostQueryPredicate = (addressOrCredential) => {
...
var Blockfrost = class {
  ...
  async getTreasury() {
    ...
    if (!result || result.error || result.supply?.treasury === void 0) {
  ...
  async getRewardAccount(rewardAddress) {
  ...
  async evaluateTx(tx, additionalUTxOs) {
    const evaluate = async (utxos) => {
      const payload = {
        cbor: tx,
        ...utxos?.length ? { additionalUtxoSet: toAditionalUTXOs(utxos) } : {}
      };
      ...
      const blockfrostRedeemer2 = res;
      if (!("EvaluationResult" in blockfrostRedeemer2.result)) {
        throw new Error(
          `EvaluateTransaction fails: ${JSON.stringify(blockfrostRedeemer2.result)}`
        );
      }
      return blockfrostRedeemer2;
    };
    const blockfrostRedeemer = additionalUTxOs?.length ? await evaluate().catch(() => evaluate(additionalUTxOs)) : await evaluate();
    const evalRedeemers = [];
...
var JSONRPCErrorSchema = import_effect2.Schema.Struct({
  code: import_effect2.Schema.Number,
  message: import_effect2.Schema.String,
  data: import_effect2.Schema.optional(import_effect2.Schema.Unknown)
});
var JSONRPCErrorResponseSchema = import_effect2.Schema.Struct({
  jsonrpc: import_effect2.Schema.String,
  method: import_effect2.Schema.optional(import_effect2.Schema.String),
  id: import_effect2.Schema.NullOr(import_effect2.Schema.Number),
  error: JSONRPCErrorSchema
});
var JSONRPCResponseSchema = (schema) => import_effect2.Schema.Union(JSONRPCSchema(schema), JSONRPCErrorResponseSchema).annotations({
  identifier: "JSONRPCResponseSchema"
});
var getJSONRPCResult = (response, operation, transport) => {
...
var LovelaceAmount = import_effect2.Schema.Union(import_effect2.Schema.Number, import_effect2.Schema.String, import_effect2.Schema.BigInt);
var TreasuryAndReservesSchema = import_effect2.Schema.Struct({
  treasury: import_effect2.Schema.Struct({
    ada: import_effect2.Schema.Struct({
      lovelace: LovelaceAmount
    })
  }),
  reserves: import_effect2.Schema.Struct({
    ada: import_effect2.Schema.Struct({
      lovelace: LovelaceAmount
    })
  })
});
var lovelaceAmountToBigInt = (lovelace) => {
...
var MaxReferenceScriptsSizeSchema = import_effect2.Schema.Struct({
  bytes: import_effect2.Schema.Number
});
function normalizeProtocolParameters(value) {
  const legacy = value.maxReferenceScriptsSize;
  const current = value.maxReferenceScriptsSizePerTransaction;
  if (legacy === void 0 && current === void 0) {
    return value;
  }
  const decode = import_effect2.Schema.decodeUnknownSync(MaxReferenceScriptsSizeSchema);
  const legacyValue = legacy === void 0 ? void 0 : decode(legacy);
  const currentValue = current === void 0 ? void 0 : decode(current);
  if (legacyValue !== void 0 && currentValue !== void 0 && legacyValue.bytes !== currentValue.bytes) {
    throw new Error(
...
var RewardAccountAdaSchema = import_effect2.Schema.Struct({
  ada: LovelaceAsset
});
var LegacyDelegationSummarySchema = import_effect2.Schema.Struct({
  delegate: import_effect2.Schema.optional(import_effect2.Schema.NullOr(import_effect2.Schema.Struct({ id: import_effect2.Schema.String }))),
  rewards: RewardAccountAdaSchema,
  deposit: RewardAccountAdaSchema
});
var RewardAccountSummarySchema = import_effect2.Schema.Struct({
  from: import_effect2.Schema.optional(import_effect2.Schema.Literal("key", "verificationKey", "script")),
  credential: import_effect2.Schema.optional(import_effect2.Schema.String),
  stakePool: import_effect2.Schema.optional(import_effect2.Schema.NullOr(import_effect2.Schema.Struct({ id: import_effect2.Schema.String }))),
  rewards: RewardAccountAdaSchema,
  deposit: RewardAccountAdaSchema
});
var Delegation = import_effect2.Schema.Union(
  import_effect2.Schema.Null,
  import_effect2.Schema.Record({
    key: import_effect2.Schema.String,
    value: LegacyDelegationSummarySchema
  }),
  import_effect2.Schema.Array(RewardAccountSummarySchema)
);
...
var QuantitySchema = import_effect3.Schema.Union(
  import_effect3.Schema.Number.pipe(import_effect3.Schema.filter(Number.isSafeInteger)),
  import_effect3.Schema.String.pipe(import_effect3.Schema.pattern(/^\d+$/))
);
var ValueSchema = import_effect3.Schema.Struct({
  coins: QuantitySchema,
  assets: import_effect3.Schema.Record({ key: import_effect3.Schema.String, value: QuantitySchema })
});
var PointSchema = import_effect3.Schema.Struct({
...
var UTxOFields = {
  ...
  spent_at: import_effect3.Schema.NullOr(PointSchema)
};
var UTxOSchema = import_effect3.Schema.Struct(UTxOFields);
var ResolvedUTxOSchema = import_effect3.Schema.Struct({
  ...UTxOFields,
  datum: import_effect3.Schema.NullOr(import_effect3.Schema.String),
  script: ScriptSchema
});
...
var makePostAsJsonResponse = (url, data, schema, headers) => (0, import_effect4.pipe)(
  import_platform.HttpClientRequest.post(url),
  headers ? import_platform.HttpClientRequest.setHeaders(headers) : import_effect4.identity,
  import_platform.HttpClientRequest.bodyJson(data),
  import_effect4.Effect.flatMap(import_platform.HttpClient.execute),
  import_effect4.Effect.flatMap(
    (response) => (0, import_effect4.pipe)(
      import_platform.HttpClientResponse.schemaBodyJson(schema)(response),
      import_effect4.Effect.map((body) => ({
        body,
        status: response.status,
        headers: response.headers
      })),
      import_effect4.Effect.catchAll(
        (cause) => response.status >= 200 && response.status < 300 ? import_effect4.Effect.fail(cause) : import_effect4.Effect.fail(
          new import_platform.HttpClientError.ResponseError({
            response,
            request: response.request,
            reason: "StatusCode",
            description: "non 2xx status code with an invalid JSON response"
          })
        )
      )
    )
...
var DEFAULT_AWAIT_TX_TIMEOUT_MS = 16e4;
var validateTimeout = (name, timeout) => {
  if (!Number.isFinite(timeout) || timeout <= 0) {
    throw new TypeError(`${name} must be a positive finite number.`);
  }
  return timeout;
};
var catchProviderError = (protocol, operation) => (effect) => import_effect6.Effect.catchAll(
  effect,
  (cause) => import_effect6.Effect.fail(toKupmiosError(cause, protocol, operation))
);
var runProviderEffect = async (effect, protocol, operation, signal) => {
  const exit = await import_effect6.Effect.runPromiseExit(effect, { signal });
  if (import_effect6.Exit.isSuccess(exit)) return exit.value;
  if (signal?.aborted) {
    throw new KupmiosError({
      protocol,
      operation,
      kind: "aborted",
      retryable: false,
      cause: signal.reason ?? import_effect6.Cause.squash(exit.cause)
    });
  }
  throw toKupmiosError(import_effect6.Cause.squash(exit.cause), protocol, operation);
};
var Kupmios = class {
  kupoUrl;
  ogmiosUrl;
  ...
  requestTimeoutMs;
  awaitTxTimeoutMs;
  constructor(kupoUrl, ogmiosUrl, options = {}) {
  ...
  async getProtocolParameters() {
    ...
    const schema = JSONRPCResponseSchema(
      import_effect5.Schema.Record({ key: import_effect5.Schema.String, value: import_effect5.Schema.Unknown })
    );
    const response = await runProviderEffect(
      (0, import_effect6.pipe)(
        makePostAsJsonResponse(
          this.ogmiosUrl,
          data,
          schema,
          this.options.ogmiosHeader
        ),
        import_effect6.Effect.timeout(this.requestTimeoutMs),
    ...
    const result = getJSONRPCResult(
      response.body,
      data.method,
      response
    );
    const protocolParameters = import_effect5.Schema.decodeUnknownSync(
      ProtocolParametersSchema
    )(normalizeProtocolParameters(result));
  ...
  async getTreasury() {
    ...
    return lovelaceAmountToBigInt(result.treasury.ada.lovelace);
  }
  async getUtxos(addressOrCredential) {
    ...
    const schema = import_effect5.Schema.Array(ResolvedUTxOSchema);
    const utxos = await runProviderEffect(
      (0, import_effect6.pipe)(
        makeGet(pattern, schema, this.options.kupoHeader),
        import_effect6.Effect.map(kupmiosUtxosToUtxos),
        import_effect6.Effect.timeout(this.requestTimeoutMs),
  ...
  async getUtxosWithPolicy(addressOrCredential, policyId) {
  ...
  async getUtxosByOutRef(outRefs) {
    ...
    const mkPattern = (txHash) => `${this.kupoUrl}/matches/*@${txHash}?unspent&resolve_hashes`;
    const schema = import_effect5.Schema.Array(ResolvedUTxOSchema);
    const program = import_effect6.Effect.forEach(
      queryHashes,
      (txHash) => (0, import_effect6.pipe)(
        makeGet(mkPattern(txHash), schema, this.options.kupoHeader),
        import_effect6.Effect.map(kupmiosUtxosToUtxos),
        import_effect6.Effect.timeout(this.requestTimeoutMs),
  ...
  async awaitTx(txHash, checkInterval = 2e4) {
    ...
    const result = await runProviderEffect(
      (0, import_effect6.pipe)(
        ...
        import_effect6.Effect.timeout(this.awaitTxTimeoutMs),
  ...
  async evaluateTx(tx, additionalUTxOs) {
    ...
    const schema = JSONRPCResponseSchema(import_effect5.Schema.Array(RedeemerSchema));
    const response = await runProviderEffect(
      (0, import_effect6.pipe)(
        makePostAsJsonResponse(
          this.ogmiosUrl,
...
var toRewardAccountState = (result) => {
  const delegation = result ? Array.isArray(result) ? result[0] : Object.values(result)[0] : void 0;
  if (!delegation) {
    return { registered: false, poolId: null, rewards: 0n };
  }
  return {
...
var toScriptRef = (script) => {
  ...
  switch (script.language) {
    ...
    case "plutus:v1":
      return {
        type: "PlutusV1",
        script: (0, import_utils4.applyDoubleCborEncoding)(script.script)
      };
    case "plutus:v2":
      return {
        type: "PlutusV2",
...
var kupmiosUtxosToUtxos = (utxos) => utxos.map((utxo) => ({
  txHash: utxo.transaction_id,
  outputIndex: utxo.output_index,
  address: utxo.address,
  assets: toAssets(utxo.value),
  datumHash: utxo.datum_type === "hash" ? utxo.datum_hash : void 0,
  datum: utxo.datum_type === "inline" ? utxo.datum : void 0,
...
var EMULATOR_PROVIDER_BRAND = Symbol.for(
  "@lucid-evolution/provider/Emulator"
);
var isEmulatorProvider = (provider) => Boolean(
  provider && provider[EMULATOR_PROVIDER_BRAND]
);
...
var Emulator = class {
  ...
  treasury;
  transactionHistory = {};
  constructor(accounts, protocolParameters = import_utils5.PROTOCOL_PARAMETERS_DEFAULT, treasury = 0n) {
  ...
  confirmPendingTransactions(blockHeight, slot) {
  ...
  getRewardAccount(rewardAddress) {
  ...
  getTransactionStatus(txHash, options = {}) {
  ...
  submitTx(tx) {
    ...
    if (currentTreasuryValue === void 0 && treasuryDonation !== void 0 || currentTreasuryValue !== void 0 && treasuryDonation === void 0) {
      throw new Error(
        "Treasury donation transactions must include both current treasury value and donation."
      );
    }
    if (treasuryDonation !== void 0) {
...
It does not exist or was already spent.`
    ...
    const certRequests = [];
    const credentialToCore = (credential) => credential.kind() === 0 ? { type: "Key", hash: credential.as_pub_key().to_hex() } : { type: "Script", hash: credential.as_script().to_hex() };
    const checkCertCredential = (credential, index) => {
      if (!credential) return;
      checkAndConsumeHash(credentialToCore(credential), "Cert", index);
    };
    const rewardAddressFromCredential = (credential) => CML.RewardAddress.new(CML.NetworkInfo.testnet().network_id(), credential).to_address().to_bech32(void 0);
...
var Maestro = class {
  ...
  maestroUtxoToUtxo(result) {
    return {
      ...
      scriptRef: toScriptRef2(result.reference_script)
    };
  }
...
var TotalsSchema = import_effect7.Schema.Struct({
  treasury: import_effect7.Schema.String
});
...
var AccountInfoSchema = import_effect7.Schema.Struct({
  status: import_effect7.Schema.Literal("registered", "not registered"),
  delegated_pool: import_effect7.Schema.NullOr(import_effect7.Schema.String),
  rewards_available: import_effect7.Schema.String
});
var TxStatusSchema = import_effect7.Schema.Struct({
  tx_hash: import_effect7.Schema.String,
  num_confirmations: import_effect7.Schema.NullOr(import_effect7.Schema.Number)
});
...
var toUTxO = (koiosUTxO, address) => ({
  ...
  scriptRef: toScriptRef3(koiosUTxO.reference_script)
});
var toScriptRef3 = (reference_script) => {
...
var Koios = class {
  ...
  async getTreasury() {
    ...
    const schema = import_effect8.Schema.Array(TotalsSchema);
    const bearerToken = this.token ? { Authorization: `Bearer ${this.token}` } : void 0;
    const [result] = await (0, import_effect8.pipe)(
      makeGet(url, schema, bearerToken),
      import_effect8.Effect.provide(import_platform4.FetchHttpClient.layer),
      import_effect8.Effect.flatMap(
        (result2) => result2.length === 0 ? import_effect8.Effect.fail("No totals found") : import_effect8.Effect.succeed(result2)
      ),
      import_effect8.Effect.timeout(1e4),
      import_effect8.Effect.catchAllCause((cause) => new KoiosError({ cause })),
      import_effect8.Effect.runPromise
  ...
  async getTransactionStatus(txHash, options = {}) {
    ...
    const result = import_effect8.Schema.decodeUnknownSync(import_effect8.Schema.Array(TxStatusSchema))(
      await response.json()
    );
