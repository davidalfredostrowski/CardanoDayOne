...
var fromAssets = (assets) => pipe(Record.remove(assets, "lovelace"), (nativeAssets) => {
  const result = {};
...
import { Cause, Effect as Effect2, Exit, pipe as pipe3, Array as _Array, Schedule } from "effect";
...
var JSONRPCErrorSchema = S.Struct({
  code: S.Number,
  message: S.String,
  data: S.optional(S.Unknown)
});
var JSONRPCErrorResponseSchema = S.Struct({
  jsonrpc: S.String,
  method: S.optional(S.String),
  id: S.NullOr(S.Number),
  error: JSONRPCErrorSchema
});
var JSONRPCResponseSchema = (schema) => S.Union(JSONRPCSchema(schema), JSONRPCErrorResponseSchema).annotations({
...
var LovelaceAmount = S.Union(S.Number, S.String, S.BigInt);
var TreasuryAndReservesSchema = S.Struct({
  treasury: S.Struct({
    ada: S.Struct({
...
var MaxReferenceScriptsSizeSchema = S.Struct({
  bytes: S.Number
});
...
var RewardAccountAdaSchema = S.Struct({
  ada: LovelaceAsset
});
var LegacyDelegationSummarySchema = S.Struct({
  delegate: S.optional(S.NullOr(S.Struct({ id: S.String }))),
  rewards: RewardAccountAdaSchema,
  deposit: RewardAccountAdaSchema
});
var RewardAccountSummarySchema = S.Struct({
...
var QuantitySchema = S2.Union(
  S2.Number.pipe(S2.filter(Number.isSafeInteger)),
  S2.String.pipe(S2.pattern(/^\d+$/))
);
var ValueSchema = S2.Struct({
  coins: QuantitySchema,
  assets: S2.Record({ key: S2.String, value: QuantitySchema })
});
var PointSchema = S2.Struct({
  slot_no: S2.Number,
...
var UTxOFields = {
  ...
  spent_at: S2.NullOr(PointSchema)
};
var UTxOSchema = S2.Struct(UTxOFields);
var ResolvedUTxOSchema = S2.Struct({
  ...UTxOFields,
  datum: S2.NullOr(S2.String),
  script: ScriptSchema
});
...
var makePostAsJsonResponse = (url, data, schema, headers) => pipe2(
  HttpClientRequest.post(url),
  headers ? HttpClientRequest.setHeaders(headers) : identity,
  HttpClientRequest.bodyJson(data),
  Effect.flatMap(HttpClient.execute),
  Effect.flatMap(
    (response) => pipe2(
      ...
      Effect.catchAll(
        (cause) => response.status >= 200 && response.status < 300 ? Effect.fail(cause) : Effect.fail(
          new HttpClientError.ResponseError({
            response,
...
var catchProviderError = (protocol, operation) => (effect) => Effect2.catchAll(
  effect,
  (cause) => Effect2.fail(toKupmiosError(cause, protocol, operation))
);
var runProviderEffect = async (effect, protocol, operation, signal) => {
  const exit = await Effect2.runPromiseExit(effect, { signal });
...
var Kupmios = class {
  ...
  async getProtocolParameters() {
    ...
    const schema = JSONRPCResponseSchema(
      S3.Record({ key: S3.String, value: S3.Unknown })
    );
    const response = await runProviderEffect(
      pipe3(
        ...
        Effect2.timeout(this.requestTimeoutMs),
    ...
    const protocolParameters = S3.decodeUnknownSync(
  ...
  async awaitTx(txHash, checkInterval = 2e4) {
    ...
    const result = await runProviderEffect(
      pipe3(
        ...
        Effect2.timeout(this.awaitTxTimeoutMs),
  ...
  async evaluateTx(tx, additionalUTxOs) {
    ...
    const schema = JSONRPCResponseSchema(S3.Array(RedeemerSchema));
    const response = await runProviderEffect(
      pipe3(
        makePostAsJsonResponse(
          this.ogmiosUrl,
...
var toScriptRef = (script) => {
  ...
  switch (script.language) {
    ...
    case "plutus:v1":
      return {
        type: "PlutusV1",
        script: applyDoubleCborEncoding2(script.script)
      };
    case "plutus:v2":
      return {
        type: "PlutusV2",
...
var Emulator = class {
  ...
  treasury;
  transactionHistory = {};
  constructor(accounts, protocolParameters = PROTOCOL_PARAMETERS_DEFAULT, treasury = 0n) {
...
var AccountInfoSchema = S4.Struct({
  status: S4.Literal("registered", "not registered"),
  delegated_pool: S4.NullOr(S4.String),
  rewards_available: S4.String
});
var TxStatusSchema = S4.Struct({
  tx_hash: S4.String,
  num_confirmations: S4.NullOr(S4.Number)
});
...
// src/koios.ts
import { Data, Effect as Effect4, pipe as pipe5, Schedule as Schedule2, Schema as S5 } from "effect";
import { FetchHttpClient as FetchHttpClient3 } from "@effect/platform";
var KoiosError = class extends Data.TaggedError("KoiosError") {
  get message() {
    return `${this.cause}`;
  }
};
var Koios = class {
  ...
  async getTreasury() {
    ...
    const schema = S5.Array(TotalsSchema);
    const bearerToken = this.token ? { Authorization: `Bearer ${this.token}` } : void 0;
    const [result] = await pipe5(
      makeGet(url, schema, bearerToken),
      Effect4.provide(FetchHttpClient3.layer),
      Effect4.flatMap(
        (result2) => result2.length === 0 ? Effect4.fail("No totals found") : Effect4.succeed(result2)
      ),
      Effect4.timeout(1e4),
      Effect4.catchAllCause((cause) => new KoiosError({ cause })),
      Effect4.runPromise
  ...
  async getTransactionStatus(txHash, options = {}) {
    ...
    const result = S5.decodeUnknownSync(S5.Array(TxStatusSchema))(
      await response.json()
    );
