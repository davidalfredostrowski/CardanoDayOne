...
var AddressType = /* @__PURE__ */ ((AddressType3) => {
  AddressType3[AddressType3["Enterprise"] = 0] = "Enterprise";
  AddressType3[AddressType3["Base"] = 1] = "Base";
  AddressType3[AddressType3["Reward"] = 2] = "Reward";
  return AddressType3;
})(AddressType || {});
...
import { generateMnemonic, mnemonicToEntropy as mnemonicToEntropy2 } from "bip39";
...
function cborKeyIsText(k, text) {
  return k instanceof CborText && k.text === text;
}
function cborKeyIsInt(k, n) {
  return (k instanceof CborUInt || k instanceof CborNegInt) && k.num === BigInt(n);
}
function cborIsSimpleTrue(v) {
...
var CoseSign1 = class _CoseSign1 {
  constructor(payload) {
    ...
    if (!this.unProtectedMap.map.find((value) => cborKeyIsText(value.k, "hashed"))) {
  ...
  hashPayload() {
    ...
    if (!this.payload) throw Error("Invalid payload");
    const hashedIndex = this.unProtectedMap.map.findIndex(
      (value) => cborKeyIsText(value.k, "hashed")
    );
    ...
    const hash = blake2b3(this.payload.bytes, void 0, 24);
    this.payload = new CborBytes(hash);
  }
  getAddress() {
    const address = this.protectedMap.map.find(
      (value) => cborKeyIsText(value.k, "address")
    );
    if (!address) throw Error("Address not found");
    return Buffer.from(address.v.bytes);
  }
  getPublicKey() {
    const publicKey = this.protectedMap.map.find(
      (value) => cborKeyIsInt(value.k, 4)
    );
...
var CardanoHeadlessWallet = class _CardanoHeadlessWallet {
  ...
  static async fromBip32Root(config) {
    const bip323 = CardanoInMemoryBip32.fromBech32(config.bech32);
    return _CardanoHeadlessWallet.create({
      addressSource: { type: "secretManager", secretManager: bip323 },
      networkId: config.networkId,
  ...
  static async fromBip32RootHex(config) {
    const bip323 = CardanoInMemoryBip32.fromKeyHex(config.hex);
    return _CardanoHeadlessWallet.create({
      addressSource: { type: "secretManager", secretManager: bip323 },
      networkId: config.networkId,
      walletAddressType: config.walletAddressType,
  ...
  static async fromMnemonic(config) {
    const bip323 = await CardanoInMemoryBip32.fromMnemonic(
      config.mnemonic,
      config.password
    );
    return _CardanoHeadlessWallet.create({
  ...
  async getUsedAddresses() {
    if (this.fetcher) {
      ...
      if (utxos.length > 0) {
        ...
        return Array.from(usedBech32Addresses).map(
          (bech32Address) => Cardano6.Address.fromBech32(bech32Address).toBytes().toString()
        );
      }
    }
  ...
  async signData(address, data) {
    ...
    const resolvedAddress = isHexString(targetAddress) ? Cardano6.Address.fromBytes(HexBlob6(targetAddress)) : Cardano6.Address.fromBech32(targetAddress);
    if (!resolvedAddress) {
...
var MeshCardanoHeadlessWallet = class _MeshCardanoHeadlessWallet extends CardanoHeadlessWallet {
  ...
  async getCollateralMesh() {
    ...
    return collaterals.map(
      (coll) => fromTxUnspentOutput(
        Serialization7.TransactionUnspentOutput.fromCbor(HexBlob7(coll))
      )
    );
  }
  /**
  ...
  async getUsedAddressesBech32() {
    ...
    return addresses.map((addr) => {
      const cardanoAddr = Cardano7.Address.fromBytes(HexBlob7(addr));
      return cardanoAddr.toBech32();
    });
  }
  /**
  ...
  async getChangeAddressBech32() {
    ...
    const cardanoAddr = Cardano7.Address.fromBytes(HexBlob7(address));
    return cardanoAddr.toBech32();
  }
  /**
   * Get the reward address for the wallet.
...
var Cip30APIError = class extends Error {
...
var Cip30TxSignError = class extends Error {
  constructor(code, info) {
...
var Cip30TxSendError = class extends Error {
  constructor(code, info) {
...
var Cip30DataSignError = class extends Error {
  constructor(code, info) {
...
var Cip30PaginateError = class extends Error {
  constructor(maxSize) {
...
function isTxSignError(error) {
  return error instanceof Cip30TxSignError;
}
function isTxSendError(error) {
  return error instanceof Cip30TxSendError;
}
function isDataSignError(error) {
  return error instanceof Cip30DataSignError;
}
function isPaginateError(error) {
  return error instanceof Cip30PaginateError;
}

// src/cardano/cip30/cip30-api.ts
import { Cardano as Cardano8, Serialization as Serialization8 } from "@cardano-sdk/core";
import { Cbor as Cbor2, CborUInt as CborUInt3 } from "@harmoniclabs/cbor";
function errorMessage(error) {
  return error instanceof Error ? error.message : String(error);
}
async function wrapApiErrors(fn) {
  try {
    return await fn();
  } catch (error) {
    if (error instanceof Cip30APIError || error instanceof Cip30TxSignError || error instanceof Cip30TxSendError || error instanceof Cip30DataSignError || error instanceof Cip30PaginateError) {
      throw error;
    }
    throw new Cip30APIError(-2 /* InternalError */, errorMessage(error));
  }
}
...
function valueCovers(total, target) {
  if (total.coin() < target.coin()) {
    return false;
  }
  const targetMultiasset = target.multiasset();
  if (targetMultiasset) {
    const totalMultiasset = total.multiasset() ?? /* @__PURE__ */ new Map();
...
function selectUtxosForAmount(utxosHex, target) {
  let total = new Serialization8.Value(0n);
  const selected = [];
  for (const utxoHex of utxosHex) {
    if (valueCovers(total, target)) {
      break;
    }
    const utxo = Serialization8.TransactionUnspentOutput.fromCbor(utxoHex);
    total = mergeValue(total, utxo.output().amount());
    selected.push(utxoHex);
  }
  return valueCovers(total, target) ? selected : null;
}
function decodeCborCoin(hex) {
  const decoded = Cbor2.parse(hex);
  if (decoded instanceof CborUInt3) {
    return decoded.num;
  }
  throw new Error(`Expected a plain CBOR unsigned int, got: ${hex}`);
}
var DEFAULT_COLLATERAL_LOVELACE = 5000000n;
function isScriptAddress(addressStr) {
  let address = null;
  try {
    address = Cardano8.Address.fromString(addressStr);
  } catch {
  }
  if (!address) {
    throw new Cip30APIError(
  ...
  const credential = props.type === Cardano8.AddressType.RewardKey || props.type === Cardano8.AddressType.RewardScript ? props.paymentPart ?? props.delegationPart : props.paymentPart;
  return credential?.type === Cardano8.CredentialType.ScriptHash;
}
function createCip30Api(wallet, opts) {
  ...
  return {
    ...
    async getUtxos(amount, paginate) {
      return wrapApiErrors(async () => {
        ...
        if (amount !== void 0) {
          let target;
          try {
            target = Serialization8.Value.fromCbor(amount);
          } catch (error) {
            throw new Cip30APIError(
              -1 /* InvalidRequest */,
    ...
    async getCollateral(params) {
      return wrapApiErrors(async () => {
        ...
        const utxosHex = await wallet.getUtxos();
        const pureAda = utxosHex.map((hex) => ({
          hex,
          utxo: Serialization8.TransactionUnspentOutput.fromCbor(hex)
        })).filter(
          ({ utxo }) => utxo.output().amount().multiasset() === void 0
        ).sort(
          (a, b) => Number(a.utxo.output().amount().coin()) - Number(b.utxo.output().amount().coin())
        );
        let total = 0n;
        const selected = [];
...
function createCip30Wallet(options) {
  ...
  return {
    ...
    async enable(args) {
      ...
      const granted = requested.filter(
        (requestedExt) => supportedExtensions.some(
          (supportedExt) => supportedExt.cip === requestedExt.cip
        )
      );
      const approved = options.approve ? await options.approve(granted) : autoApprove;
      if (!approved) {
        throw new Cip30APIError(
          -3 /* Refused */,
...
// src/cardano/cip30/deserializers.ts
import { Cardano as Cardano9, Serialization as Serialization9 } from "@cardano-sdk/core";
function deserializeWalletAddressFromHex(hex) {
  return {
    hex,
    bech32: Cardano9.Address.fromBytes(hex).toBech32()
  };
}
function deserializeWalletMultiasset(multiasset) {
  if (!multiasset) return [];
  return Array.from(multiasset.entries()).map(([assetId, quantity]) => {
    const policyId2 = assetId.slice(0, 56);
    const assetName2 = assetId.slice(56);
    return { policyId: policyId2, assetName: assetName2, quantity, unit: policyId2 + assetName2 };
  });
}
function deserializeWalletBalanceFromCbor(cbor) {
  const v = Serialization9.Value.fromCbor(cbor);
  return { cbor, lovelace: v.coin(), assets: deserializeWalletMultiasset(v.multiasset()) };
}
function deserializeWalletUtxoFromCbor(cbor) {
  const u = Serialization9.TransactionUnspentOutput.fromCbor(cbor);
  const input = u.input();
  const output = u.output();
  const amount = [
    { unit: "lovelace", quantity: String(output.amount().coin()) },
    ...Array.from(output.amount().multiasset()?.entries() ?? []).map(
      ([assetId, qty]) => ({ unit: assetId, quantity: String(qty) })
    )
  ];
  const utxo = {
    input: {
  ...
  if (scriptRef !== void 0) {
    utxo.output.scriptHash = scriptRef.hash();
  }
  return utxo;
}
function deserializeWalletUtxosFromCbor(cbors) {
  return cbors === null ? null : cbors.map(deserializeWalletUtxoFromCbor);
}

// src/bitcoin/interfaces/bitcoin-wallet.ts
...
// src/bitcoin/utils/coin-selection.ts
var DUST_THRESHOLD_P2WPKH = 546;
function selectUtxosLargestFirst(utxos, targetAmount, feeRate, numRecipients) {
...
// src/bitcoin/wallet/core/bitcoin-core.ts
import * as bitcoin from "bitcoinjs-lib";
import * as ecc2 from "tiny-secp256k1";
import * as bip39 from "bip39";
import { BIP32Factory as BIP32Factory2 } from "bip32";
import { ECPairFactory } from "ecpair";
var bip322 = BIP32Factory2(ecc2);
var ECPair = ECPairFactory(ecc2);
bitcoin.initEccLib(ecc2);

// src/bitcoin/address/bitcoin-address.ts
...
function deriveP2wpkhAddress(publicKey, network) {
...
function deriveP2trAddress(publicKey, network) {
  const pubkey = Buffer.isBuffer(publicKey) ? publicKey : Buffer.from(publicKey);
...
var DerivedBitcoinAddress = class {
  constructor(args) {
...
// src/bitcoin/address/bitcoin-address-manager.ts
function getCoinType(network) {
  return network.bech32 === "bc" ? 0 : 1;
}
function paymentPath(network, account = 0, change = 0, index = 0) {
  return `m/84'/${getCoinType(network)}'/${account}'/${change}/${index}`;
}
function ordinalsPath(network, account = 0, change = 0, index = 0) {
  return `m/86'/${getCoinType(network)}'/${account}'/${change}/${index}`;
}
var BitcoinAddressManager = class _BitcoinAddressManager {
  constructor(config) {
    this.network = config.network;
    this.root = config.root;
    this.account = config.account ?? 0;
  }
  static fromSeed(seed, network, account = 0) {
    const root = bip322.fromSeed(seed, network);
    return new _BitcoinAddressManager({ network, root, account });
  }
  getNetwork() {
    return this.network;
  }
   ...
   */
  getAddress(purpose, change = 0, index = 0) {
    if (purpose === "payment" /* Payment */) {
   ...
   */
  getAddresses(purposes) {
    const list = purposes && purposes.length > 0 ? purposes : ["payment" /* Payment */, "ordinals" /* Ordinals */];
    const out = [];
    for (const purpose of list) {
      if (purpose !== "payment" /* Payment */ && purpose !== "ordinals" /* Ordinals */) {
        continue;
      }
   ...
   */
  getChild(purpose, change = 0, index = 0) {
    const path = purpose === "payment" /* Payment */ ? paymentPath(this.network, this.account, change, index) : purpose === "ordinals" /* Ordinals */ ? ordinalsPath(this.network, this.account, change, index) : null;
    if (!path) {
      throw new Error(
        `[BitcoinAddressManager] Unsupported address purpose: ${purpose}`
      );
   ...
   */
  getAddressesByPurpose(purpose, start = 0, count = 20, change = 0) {
    if (count <= 0) return [];
    return Array.from(
      { length: count },
      (_, i) => this.getAddress(purpose, change, start + i)
    );
  }
  /**
   ...
   */
  findAddress(address, purpose, maxGap = 20) {
   ...
   */
  getAccountNode(purpose) {
    if (purpose !== "payment" /* Payment */ && purpose !== "ordinals" /* Ordinals */) {
      throw new Error(
        `[BitcoinAddressManager] getAccountNode: unsupported purpose "${purpose}". Use AddressPurpose.Payment (BIP-84) or AddressPurpose.Ordinals (BIP-86).`
      );
    }
    const coinType = getCoinType(this.network);
    const path = purpose === "payment" /* Payment */ ? `m/84'/${coinType}'/${this.account}'` : `m/86'/${coinType}'/${this.account}'`;
   ...
   */
  getAccountXpub() {
    return this.getAccountNode("payment" /* Payment */).toBase58();
  }
  /**
   ...
   */
  getTaprootXpub() {
    return this.getAccountNode("ordinals" /* Ordinals */).toBase58();
  }
  /**
  ...
  getAccountZpub() {
    ...
    const payload = Buffer.from(esm_default3.decode(xpub));
    const isMainnet = this.network.bech32 === "bc";
    const zpubVersion = isMainnet ? Buffer.from([4, 178, 71, 70]) : Buffer.from([4, 95, 28, 246]);
    zpubVersion.copy(payload, 0);
    return esm_default3.encode(payload);
  }
};

// src/bitcoin/wallet/mesh/bitcoin-headless-wallet.ts
var BitcoinHeadlessWallet = class _BitcoinHeadlessWallet {
  constructor(cfg) {
  ...
  static async create(config) {
    ...
    return new _BitcoinHeadlessWallet({
  ...
  static async fromMnemonic(config) {
    ...
    const bitcoinNetwork = networkFromName(config.network);
    const root = bip322.fromSeed(seed, bitcoinNetwork);
    return _BitcoinHeadlessWallet.create({ ...config, root });
  }
  /**
   * Create a headless wallet from BIP-39 entropy (hex string).
   */
  static async fromEntropy(config) {
  ...
  static async fromPrivateKey(config) {
    ...
    if (/^[0-9a-fA-F]{64}$/.test(config.privateKey)) {
      ...
      try {
        ...
        throw new Error(
          `[BitcoinHeadlessWallet] privateKey must be a 32-byte hex string or valid WIF: ${e.message}`
        );
      }
    }
    ...
    const root = bip322.fromSeed(privKeyBuffer, bitcoinNetwork);
    return _BitcoinHeadlessWallet.create({ ...config, root });
  }
  // ---------------------------------------------------------------------------
  // IBitcoinWallet
  ...
  async getAddresses(addressPurposes) {
    return this.manager.getAddresses(addressPurposes).map((a) => a.toBitcoinAddress());
  }
  async getAccounts(addressPurposes) {
    return this.manager.getAddresses(addressPurposes).map((a) => a.toBitcoinAccount());
  }
  async getBalance() {
    if (!this.provider) {
      throw new Error(
    ...
    const addressInfos = await Promise.all(
      paymentAddresses.map(
        (address) => this.provider.fetchAddressInfo(address.address)
      )
    );
    const { confirmed, unconfirmed } = addressInfos.reduce(
      (acc, info) => {
        acc.confirmed += info.chain_stats.funded_txo_sum - info.chain_stats.spent_txo_sum;
        acc.unconfirmed += info.mempool_stats.funded_txo_sum - info.mempool_stats.spent_txo_sum;
        return acc;
      },
      {
        confirmed: 0,
   ...
   */
  findManagedAddress(address, purpose, maxGap = 20) {
  ...
  async signMessage(address, message, protocol = "ECDSA" /* ECDSA */) {
    if (protocol === "BIP322" /* BIP322 */) {
      throw new Error(
    ...
    const rawSig = Buffer.from(ecc2.sign(hash, privateKey));
  ...
  async signTransfer(recipients) {
    ...
    const paymentChild = this.manager.getChild("payment" /* Payment */);
  ...
  async signPsbt(signConfig) {
    ...
    const addressToSigner = /* @__PURE__ */ new Map();
    for (const purpose of ["payment" /* Payment */, "ordinals" /* Ordinals */]) {
    ...
    const indexesUsed = /* @__PURE__ */ new Set();
  ...
  signerForPurpose(purpose, change = 0, index = 0) {
    ...
    const tweakedFullPub = ecc2.pointFromScalar(tweakedPrivateKey, true);
    if (!tweakedFullPub) {
      throw new Error(
    ...
    const signer = {
      ...
      signSchnorr: (hash) => {
        const sig = ecc2.signSchnorr(hash, tweakedPrivateKey);
        return Buffer.from(sig);
      }
    };
    return signer;
  }
  signSingleInput(psbt, index, entry) {
    if (entry.purpose === "ordinals" /* Ordinals */) {
      ...
      psbt.signInput(index, entry.signer, [
...
function buildDefaultSignTargets(psbt, addressToSigner) {
  const purposeToAddress = /* @__PURE__ */ new Map();
  ...
  for (let i = 0; i < psbt.inputCount; i++) {
    ...
    } else if (script.length === 34 && script[0] === 81 && script[1] === 32) {
      purpose = "ordinals" /* Ordinals */;
    }
    if (!purpose) continue;
    const address = purposeToAddress.get(purpose);
    if (!address) continue;
    (out[address] ?? (out[address] = [])).push(i);
  }
  return out;
}
function findRecoveryId(hash, sig, expectedPubkey) {
  for (const rid of [0, 1, 2, 3]) {
    const recovered = ecc2.recover(hash, sig, rid, true);
...
function verifyBitcoinMessage(address, message, signature, network) {
  let decoded;
  try {
  ...
  const hash = bitcoinMessageHash(message);
  const recovered = ecc2.recover(hash, rawSig, recoveryId, compressed);
  if (!recovered) {
    return {
      valid: false,
  ...
  if (!ecc2.verify(hash, recoveredPubkey, rawSig)) {
    return {
      valid: false,
...
function varIntBuffer(n) {
  ...
  if (n <= 65535) return Buffer.from([253, n & 255, n >> 8]);
  if (n <= 4294967295)
    return Buffer.from([
      254,
      n & 255,
...
function tweakPrivateKey(privateKey, internalPubkey) {
  const fullPub = ecc2.pointFromScalar(privateKey, true);
  if (!fullPub) {
    throw new Error("[BitcoinHeadlessWallet] Invalid private key");
  }
  let priv = privateKey;
  if (fullPub[0] === 3) {
    const negated = ecc2.privateNegate(privateKey);
    if (!negated) {
  ...
  const tweaked = ecc2.privateAdd(priv, taggedHash);
  if (!tweaked) {
    throw new Error(
...
// src/bitcoin/wallet/browser/adapters/xverse-adapter.ts
var XVERSE_ACCESS_DENIED = -32002;
var XverseRpcError = class extends Error {
...
async function call(method, params) {
  const response = await getProvider().request(method, params ?? null);
...
var DEFAULT_CONNECT_MESSAGE = "Connect to Mesh SDK";
function normalizeAddressType(t) {
  const lower = t.toLowerCase();
  const known = Object.values(AddressType2);
  if (known.includes(lower)) return lower;
  if (lower === "p2sh-p2wpkh") return "p2sh";
  return null;
}
function normalizePurpose(raw) {
  const known = Object.values(AddressPurpose);
  if (known.includes(raw)) return raw;
  return null;
}
function mapAddressItems(items) {
  const out = [];
  for (const a of items) {
    ...
    out.push({
      ...
      purpose,
      addressType,
      walletType: a.walletType ?? "software"
    });
  }
  return out;
}
function parseAddressListResult(result) {
  if (Array.isArray(result)) return result;
  return result.addresses ?? result.accounts ?? [];
}
var XverseAdapter = class _XverseAdapter {
  constructor() {
  ...
  static async enable() {
    ...
    const adapter = new _XverseAdapter();
    await adapter.connect(["payment" /* Payment */, "ordinals" /* Ordinals */]);
   ...
   */
  async connect(purposes, message = DEFAULT_CONNECT_MESSAGE) {
    try {
      await this.connectViaWalletConnect(purposes, message);
      return;
    } catch (err) {
      if (err instanceof XverseRpcError && err.code === -32601) {
  ...
  async connectViaWalletConnect(purposes, message) {
    const result = await call("wallet_connect", {
      addresses: purposes,
      message
    });
  ...
  async connectViaPermissions(purposes, message) {
    await call("wallet_requestPermissions", null);
    await this.requestAddresses(purposes, message);
  }
  async requestPermissions() {
    await call("wallet_requestPermissions", null);
  }
  async requestAddresses(purposes, message = DEFAULT_CONNECT_MESSAGE) {
    try {
  ...
  async fetchAddresses(purposes, message) {
    const result = await call("getAddresses", { purposes, message });
  ...
  async getNetwork() {
    let result;
    try {
      result = await call("wallet_getNetwork");
    } catch (err) {
      if (err instanceof XverseRpcError && err.code === -32601) {
        result = await call("getNetwork");
      } else {
        throw err;
      }
    }
    const raw = result.name ?? result.bitcoin?.name;
  ...
  async getAccounts(addressPurposes) {
    const result = await call("getAccounts", {
      purposes: addressPurposes,
      message: "Connect to Mesh SDK"
    });
    const items = Array.isArray(result) ? result : result.accounts ?? result.addresses ?? [];
    return items.map((a) => ({
      walletType: a.walletType ?? "software",
      address: a.address,
      publicKey: a.publicKey,
      ...
      (a) => a.addressType !== null && a.purpose !== null
    );
  }
  async getBalance() {
    const result = await call("getBalance");
    if (result.confirmed == null || result.unconfirmed == null || result.total == null) {
      throw new Error(
  ...
  async signMessage(address, message, protocol) {
    const params = { address, message };
    if (protocol) params.protocol = protocol;
    const result = await call("signMessage", params);
    return {
      signature: result.signature,
      messageHash: result.messageHash,
      address: result.address,
      protocol: result.protocol ?? protocol ?? "ECDSA" /* ECDSA */
    };
  }
  /**
   ...
   */
  async signTransfer(recipients) {
    const result = await call("sendTransfer", {
  ...
  async fetchUTXOs(purposes = [
    ...
    const results = await Promise.all(
      addresses.map(async (addr) => {
        const utxos = await call("wallet_getUtxos", {
  ...
  async getTransactionHistory(options = {}) {
    ...
    const results = await Promise.all(
      addresses.map(async (addr) => {
        const txs = await call("wallet_getTransactions", {
          address: addr.address,
...
// src/bitcoin/wallet/browser/bitcoin-browser-wallet.ts
var PERSIST_KEY = "mesh_bitcoin_wallet";
function assertClientSide() {
  if (typeof window === "undefined") {
    throw new Error(
      "BitcoinBrowserWallet: This class can only be used in a browser environment. Do not import or call it during server-side rendering."
    );
  }
}
var REGISTRY = {
...
var BitcoinBrowserWallet = class _BitcoinBrowserWallet {
  constructor(walletInstance, walletName = "") {
    this.walletInstance = walletInstance;
  ...
  getAccounts(addressPurposes) {
    return this.walletInstance.getAccounts(addressPurposes);
  }
  getBalance() {
    return this.walletInstance.getBalance();
  }
  signMessage(address, message, protocol) {
   ...
   */
  static async enable(walletName, options) {
    assertClientSide();
    ...
    if (options?.persist && localStorage.getItem(PERSIST_KEY) === key) {
      try {
        ...
        return new _BitcoinBrowserWallet(instance, key);
      } catch {
        localStorage.removeItem(PERSIST_KEY);
      }
    }
    try {
      const instance = await entry.factory();
      const wallet = new _BitcoinBrowserWallet(instance, key);
      if (options?.persist) {
  ...
  static async restore() {
    ...
    try {
      return await _BitcoinBrowserWallet.enable(saved, { persist: true });
    } catch {
      localStorage.removeItem(PERSIST_KEY);
      return null;
    }
...
export {
  ...
  AddressType2 as BitcoinAddressType,
  BitcoinBrowserWallet,
  ...
  deserializeWalletBalanceFromCbor,
  deserializeWalletMultiasset,
  deserializeWalletUtxoFromCbor,
  deserializeWalletUtxosFromCbor,
  getCoinType,
  isApiError,
  isDataSignError,
  isPaginateError,
